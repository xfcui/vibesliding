...
- **Cross-domain validation — AI Drama's "spray and pray":** China's $7B short-drama industry applies identical probability logic at industrial scale — 470 AI-generated titles released per day at $6K–$15K each, only 0.117% break out past 100M views, yet the portfolio is profitable because cost per failed attempt is trivially low relative to hit upside (MIT Technology Review, 2026)
...
- **Cross-domain proof — AI video production:** Short-Drama-Bench (2026) shows pipeline-based systems (multi-agent with character DNA + first-frame lock) outperform end-to-end single-model generation by 35–50% on Character Identity Score and 20–30% on emotional arc coherence — OpenAI's Sora, lacking storyboard mode and character lock, was deprioritized for serialized production while pipeline-first Seedance and Kling dominate a $7B market
...
[Speech: "Why not just use Tome or Gamma?" I get this question a lot. Here's the architectural answer. Tome and Gamma are monolithic: prompt in, slides out, edit retroactively. Gamma's own blog admits users edit seventy-eight percent of generated slides. And when something's wrong — a bad metric on slide thirty-seven — you can't tell if it's a research gap, an outline error, or a rendering bug. You just see the symptom, not the cause. VibeSliding's pipeline is transparent. Every stage produces inspectable artifacts. Catching one metric error in the outline is trivially cheap. Fixing that same error across sixty rendered slides is expensive and error-prone. And this isn't unique to slides — in AI video, the same pattern plays out. OpenAI's Sora generates impressive single clips but was deprioritized for actual series production because it lacks storyboard mode and character lock. Meanwhile, pipeline-first tools like Seedance and Kling dominate a seven-billion-dollar market precisely because they fit into multi-agent workflows. Compound beats monolithic everywhere. Now let's walk through the actual pipeline — starting with deep research.]
...
- **AI Drama validation ($7B market):** China's short-drama industry independently converged on the same architecture — a seven-stage multi-agent pipeline (screenwriter → character DNA → storyboard → first-frame lock → I2V → audio/BGM → reviewer QC) producing 470 titles/day; pipeline-based systems outperform monolithic generation by 35–50% on identity consistency, validating that compound beats single-model across domains (Short-Drama-Bench, 2026)
- **NotebookLM validation (200M+ interactions/month):** Google's NotebookLM uses a six-stage compound pipeline (parse → chunk → embed → index → retrieve → rerank) with hybrid long-context/RAG switching — serving 200M+ monthly interactions through modular orchestration; the architecture was so effective that community clients exceeding 20K GitHub stars reverse-engineered its internals to replicate the pattern
...
[Speech: Here's the thesis. Models commoditize fast — we swapped one for another and got 18 percent better results with zero code changes. If the model were our moat, that would be terrifying. But the orchestration layer — the sequencing, the quality gates, the human checkpoints, the parallel-then-select pattern — that's what we built, that's what we control, and that's what compounds over time. Zaharia and colleagues at Berkeley call these "compound AI systems" — systems that achieve performance by combining components, not by relying on a single model. And the evidence is now cross-domain. China's AI drama industry — seven billion dollars — independently discovered the same architecture: a seven-stage pipeline with character DNA, storyboard agents, and QC loops. Google's NotebookLM — two hundred million interactions a month — runs a six-stage compound pipeline for source grounding. Neither copied the other. They converged on the same design because the physics of compound AI systems demands it. Replace "slides" with "video episodes" or "research notebooks" and the pipeline still works. The scaffold alone improved visual consistency by 61 percent — independent of which model generated the content. Invest in orchestration. That's the layer that compounds.]
...
- **Step 2 — Run the four stages:** `src.research.cli` (Valyu DeepResearch → `research.md`) → `src.outline.cli` (emits 16/25/36-slide outlines + `style_base.md`) → `src.render.style.cli` (interactive contact-sheet picker → `style_*.png`) → `src.render.cli` (parallel render → timestamped `slides_*/` + `slide_combined.pdf`)
...
[Visual: A terminal window occupying the left two-thirds of the slide on Deep Space background, showing a short setup block (`git clone`, `pip install`, `cp .env.example .env`) followed by four sequential CLI commands in JetBrains Mono 14pt with syntax highlighting: `research.cli`, `outline.cli`, `render.style.cli`, `render.cli`. Each command has a small output preview fanning out to the right: research → citation list snippet, outline → film strip, style → contact sheet grid, compose → slide thumbnails. These previews converge into a glowing PDF icon at the far right labeled "slide_combined.pdf." Small badges float near the PDF: a "< 60 min" timer in Electric Cyan and "$4.73" cost in Signal Green (#00E676), plus an "MIT · 100% Python" repo chip and `github.com/xfcui/vibesliding` URL in JetBrains Mono 12pt along the bottom edge. Pipeline-thread watermark at 15% opacity.]
